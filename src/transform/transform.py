...
def _average_rotation(R):
    """Average rotation vectors using scipy (matches NeSVoR)."""
    dtype = R.dtype
    device = R.device
    Rmat = Rotation.from_rotvec(R.detach().cpu().numpy()).as_matrix()
    # Quaternion averaging via eigenvalue method
    quats = Rotation.from_matrix(Rmat).as_quat()  # (N, 4)
    Q = quats.T @ quats  # (4, 4)
    eigvals, eigvecs = np.linalg.eigh(Q)
    avg_quat = eigvecs[:, -1]
    avg_rotvec = Rotation.from_quat(avg_quat).as_rotvec()
    return torch.from_numpy(avg_rotvec).to(dtype=dtype, device=device).unsqueeze(0)
...
# random angle
def random_angle(n, restricted, device, i=0):
    orientations = [
        (0, 0, 0),               # axial
        (0, np.pi / 2, 0),       # coronal
        (np.pi / 2, 0, 0),       # sagittal
    ]
    base_angle = orientations[i % 3]
    if restricted:
        # small random noise around the base orientation
        noise_scale = np.deg2rad(3)  # ±3度扰动
        noise = np.random.uniform(-noise_scale, noise_scale, size=3)
    else:
        noise = np.zeros(3)

    euler_angle = np.array(base_angle) + noise
    R = Rotation.from_euler("ZXZ", np.tile(euler_angle, (n, 1)))
    rotvec = R.as_rotvec()
    return torch.from_numpy(rotvec).to(dtype=torch.float32, device=device)
...
def average_rotation(R):
    """Weiszfeld mean on SO(3) — matches NeSVoR exactly."""
    import scipy.linalg

    dtype = R.dtype
    device = R.device
    Rmat = Rotation.from_rotvec(R.detach().cpu().numpy()).as_matrix()
    Q = Rotation.from_rotvec(R.detach().cpu().numpy()).as_quat()
    for i in range(Q.shape[0]):
        if np.linalg.norm(Q[i] + Q[0]) < np.linalg.norm(Q[i] - Q[0]):
            Q[i] *= -1
    barR = np.mean(Q, 0)
    ...
    while np.all(np.isreal(S_new)) and np.all(np.isfinite(S_new)) and i < 10:
        ...
        for j in range(R_np.shape[0]):
            vmatrix = scipy.linalg.logm(np.matmul(R_np[j], np.linalg.inv(S)))
